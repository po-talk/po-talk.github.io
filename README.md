...
## できること
- 🧑 プロフィール：名前と絵文字アバターを設定（ブラウザに保存、次回も引き継ぐ）
- 🗣 発話インジケータ：話している人の絵文字が声に合わせて跳ねる
- 🔇 ミュート表示：参加者リストに各自のミュート状態を表示
- 🟢 ロビー：通話中の部屋を一覧（死活監視）、タップでそのまま入室
...
- ロビーはページを開いている**閲覧者全員**がP2Pで繋がる（データのみのメッシュ）。効くのは通話人数ではなく同時閲覧者数で、**数十人規模で頭打ち**（超える見込みなら Nostr 直publish 等サーバレスな方式へ）。
