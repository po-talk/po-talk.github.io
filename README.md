...
- 📮 **おたより**（配信部屋）：聞き役が書いたお便りは**配信者にだけ届き**、配信者が選んだものだけが部屋に出ます（ラジオのハガキ募集）
- 🔊 **音量**：この通話の音だけを下げられます（動画や音楽と一緒に聞くとき用。ほかのアプリの音はそのまま）
- 🎵 **BGM**：⚙️ 設定から「軽快な音楽」か「アンビエント空間」を選べます（既定はオフ・**自分にだけ鳴ります**・曲は部屋名で決まります）
- 📱 **小窓**：通話中に小さな窓を最前面に浮かべられます。ほかのアプリを見ている間も**通話中だと分かり、タップで一発で戻れます**（ホーム画面に追加したアプリでは使えません）
- 👥 いまこのページを見ている人数のめやす
- 🌱 使うほど自分の絵文字が育つ
...
- **iPhone は「ホーム画面に追加」ではなく Safari のタブで開く**のがおすすめです。ホーム画面のアプリとして開くと、**他のアプリに切り替えた瞬間に自分の声が届かなくなります**（iOS の仕様で、こちらからは防げません）
- **音を出す他のアプリ（写真・動画など）に切り替えると、通話の音が一時的に止まります**（そのアプリの音が止まれば鳴り直します）
...
**📝 紹介していただいた記事**
- [POPOPOの代替、後継アプリを適当に書く ↗](https://wanderingdj.jp/wordpress/2026/08/17/popopo%E3%81%AE%E4%BB%A3%E6%9B%BF%E3%80%81%E5%BE%8C%E7%B6%99%E3%82%A2%E3%83%97%E3%83%AA%E3%82%92%E9%81%A9%E5%BD%93%E3%81%AB%E6%9B%B8%E3%81%8F/) — wanderingdj's（2026/08/17）／POPOPO の後継候補をいくつも見比べたうえで、ぽっと通話にも触れていただきました
...
- 2026/08/17 ブログ **[wanderingdj's ↗](https://wanderingdj.jp/wordpress/2026/08/17/popopo%E3%81%AE%E4%BB%A3%E6%9B%BF%E3%80%81%E5%BE%8C%E7%B6%99%E3%82%A2%E3%83%97%E3%83%AA%E3%82%92%E9%81%A9%E5%BD%93%E3%81%AB%E6%9B%B8%E3%81%8F/)** で紹介していただきました
- 2026/08/18 — **v0.8.1**（📮 おたより）
    - 配信部屋で**お便りを募集**できるようにしました。聞き役が書いたものは**配信者にだけ届き**、配信者が選んだものだけが部屋に出ます（ラジオのハガキ募集）
    - 公開されたお便りは**どのタブを見ていても**大きな紙で出ます。ほかの人の投稿は見えません
- 2026/08/18 — **v0.8.2〜v0.8.5**（細かい直しの回）
    - スマホでタブの文字が折り返す／普通の部屋に「おたより」タブが出てしまう問題を修正
    - 参加者リストを**左は見るだけ・右は押すもの**に整理しました
    - β（旧URL）から安定版へ移れる導線を追加。Google の検索結果でファビコンが空になるのも直しました
- 2026/08/18〜19 — **v0.8.6 / v0.8.7**（🔊 音量）
    - **この通話の音だけ**を下げられるようにしました（動画や音楽と一緒に聞くとき用）
    - ※ iPhone は音量指定を**受け付けたふりをして無視する**ため、端末を見分ける方法がありません。**全端末で同じ仕組み**（Web Audio）に統一して解決しました
- 2026/08/19 — **v0.8.8**（🎵 BGM）
    - ⚙️ 設定から **軽快な音楽**／**アンビエント空間**を選べます（既定はオフ）
    - **自分にだけ鳴ります**（相手には送っていません）。曲は**部屋名から決まる**ので、通信は一切増えません
    - 音源ファイルを持たず、その場で合成しています（＝権利関係の心配がありません）
- 2026/09/12 — **v0.8.9**（マイクの取り方を明示）
    - Android の一部機種で音が回り込む（ハウリング）報告を受けて、**エコーキャンセル等を明示的に指定**するようにしました
- 2026/09/12 — **v0.9.0**（音が止まったまま戻らない問題）
    - iPhone で**他のアプリに切り替えると、戻ってきても BGM や相手の声が鳴らないまま**になることがある問題を直しました
    - ※ 背面にいる**あいだ**に音が止まるのは iOS の仕様で防げません。直したのは**戻ったあとに鳴り直すこと**です
- 2026/09/14 — **v0.9.1〜v0.10.3**（音の出口の作り直しと 📱 小窓）
    - **iPhone で BGM や効果音がほとんど聞こえない**問題を直しました。音の出し方そのものを変えています（スピーカーへ直接ではなく、音声プレイヤーを通す形へ）
    - **📱 小窓**を追加。通話中に小さな窓を最前面に浮かべられます。ほかのアプリを見ている間も通話中だと分かり、**タップで一発で戻れます**
    - 音を出す他のアプリに切り替えて通話の音が止まっても、**自動で鳴り直す**ようにしました
    - BGM が背面で途切れることがある問題を修正
    - **音量を下げると自分の声が返ってくる**不具合を修正しました（v0.8.7 から入っていたものです）
    - 最初の画面を **「名前を決める → 部屋を選ぶ」** の流れに整理しました（部屋を新しく作る欄は畳んであります）
